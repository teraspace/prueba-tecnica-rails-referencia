class CutPlanner
  module Strategies
    class BestFitDecreasing
      BarState = Struct.new(:stock_id, :stock_index, :instance, :length, :usable_length, :remaining, :pieces, keyword_init: true)
      ...
      def call
        ...
        { bars: states.map { |state| build_bar_plan(state) }, unplaced: build_unplaced(unplaced) }
      end
      ...
      def expanded_pieces
        @input.pieces.flat_map do |piece|
          (1..piece.quantity).map do |unit_index|
            Result::PieceAssignment.new(length: piece.length, source_index: piece.source_index, unit_index: unit_index)
          end
        end.sort_by { |piece| [-piece.length, piece.source_index, piece.unit_index] }
      end

      def best_fit_bar(states, required)
        states.select { |state| state.remaining >= required }
      ...
      def open_best_fit_bar(states, required)
        available = @input.stock.select do |stock_type|
          stock_type.available > states.count { |state| state.stock_id == stock_type.id } && usable_length(stock_type) >= required
        end
        ...
        state = BarState.new(
          stock_id: stock_type.id, stock_index: stock_type.source_index, instance: instance,
          length: stock_type.length, usable_length: usable_length(stock_type),
          remaining: usable_length(stock_type), pieces: []
        )
        states << state
        state
      end
      ...
      def build_bar_plan(state)
        Result::BarPlan.new(
          stock_id: state.stock_id, instance: state.instance, length: state.length,
          usable_length: state.usable_length, pieces: state.pieces,
          leftover: state.remaining, waste: state.length - state.pieces.sum(&:length)
        )
      end

      def build_unplaced(unplaced)
        unplaced.sort_by { |source_index, _| source_index }.map do |source_index, item|
          Result::UnplacedPiece.new(length: item.fetch(:length), quantity: item.fetch(:quantity), source_index: source_index)
        end
      end
    end
  end
